...
final class FileDownloadProvider
{
    ...
    {
        if (!isset($_GET['widgetChannelId']) || !isset($_GET['entryID']) || !isset($_GET['fileID'])) {
            global $wp_query;
            $wp_query->set_404();
            status_header(404);
        ...
        $filePath = Config::get('UPLOAD_DIR') . DIRECTORY_SEPARATOR . $widgetChannelId . DIRECTORY_SEPARATOR . $entryID . DIRECTORY_SEPARATOR . $fileID;
    ...
    private function fileDownloadORView($filePath, $forceDownload = false)
    {
        if ($forceDownload) {
            ...
            $content_types = mime_content_type($filePath);
            $ext = explode('/', $content_types);
