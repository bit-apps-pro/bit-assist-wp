...
export const deepCopy = (target:object, map = new WeakMap()) => {
...
export const dateTimeFormatter = (dateStr, format) => {
  ...
  let formattedDate = ''
  const allFormatObj = {
    a, A, c, d, D, F, g, G, h, H, i, j, l, m, M, n, r, s, S, T, U, y, Y,
  }
...
export const getColorPreference = () => window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches
