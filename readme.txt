...
= v1.8.2 (Jul 19, 2026) =
* Security: Hardened the visitor file upload — executable/script file types are blocked and file contents are verified against their extension.
* Feature: SVG file uploads are now supported; uploaded SVGs are sanitized (scripts, event handlers, and external references removed) before being stored and are served with hardened headers.
* Improvement: Submitting a disallowed file now shows a clear error message naming the rejected file instead of silently dropping it.
...
= v1.7.2 (Mar 31, 2026) =
* Fix: Repaired the legacy Bit Assist Pro update cache path for Pro versions below 1.0.7.

= v1.7.1 (Mar 30, 2026) =
* Fix: File upload failing on servers with symlinked or bind-mounted upload directories.
* Fix: Header injection vulnerability in file download Content-Disposition header.

= v1.7.0 (Mar 14, 2026) =
* Feature: Translation ready -- Bit Assist is now translation-ready, allowing users to easily translate the plugin into their preferred language using standard WordPress translation tools.
